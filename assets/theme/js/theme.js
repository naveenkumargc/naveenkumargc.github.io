        ...
        if ($('.validate-form').length > 0) {
            ...
            $('.validate-form').submit(function () {
                ...
                if (el.valid()) {
                    ...
                    const preloadSubmit = document.querySelector(".preload-submit");
                    const messageSubmit = document.querySelector(".message-submit");
                    preloadSubmit.classList.remove("hidden");
                    ...
                    const formData = {
                        ...
                        subject: this.subject.value || "General Inquiry"
                    };

                    // Send email using EmailJS
                    emailjs.send("service_4cxcrs5", "template_gyhflf4", formData)
                        .then(function (response) {
                            preloadSubmit.classList.add("hidden");
                            messageSubmit.classList.remove("hidden");
                            messageSubmit.classList.remove("error");
                            messageSubmit.textContent = "Message sent successfully!";
                            document.getElementById("ContactForm").reset(); // Reset the form
                        }, function (error) {
                            preloadSubmit.classList.add("hidden");
                            messageSubmit.classList.remove("hidden");
                            messageSubmit.classList.add("error");
                            messageSubmit.textContent = "Failed to send message. Please try again.";
